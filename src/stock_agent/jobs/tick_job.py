"""jobs/tick_job.py — 实时 tick 数据采集"""
import datetime, random, time
import akshare as ak
import pandas as pd
from loguru import logger
from core.db import engine, get_job_by_date
from core.redis_client import redis_cache_df_parquet
...
def load_all_ticks_today():
    ...
    for page in pages:
        ...
        if stocks.empty:
            time.sleep(BATCH_INTERVAL + 1); continue
        for _, row in stocks.iterrows():
            ticks = get_tx_ticks(row["symbol"])
            if ticks.empty:
                time.sleep(BATCH_INTERVAL); continue
