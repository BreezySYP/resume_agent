...
# # 添加项目根目录到 sys.path
# root_dir = str(Path(__file__).resolve().parent.parent.parent.parent.parent)  # 调整层级到 workspace 根
# if root_dir not in sys.path:
#     sys.path.insert(0, root_dir)

"""core/db.py — MySQL 连接池 + 查询工具"""
import pymysql
import pandas as pd
...
from shared.configs.settings import MYSQL_ROOT_USER, MYSQL_PASSWORD, MYSQL_HOST, MYSQL_PORT, MYSQL_DB
...
def get_job_by_date(date_str: str):
    with get_connection() as conn:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT * FROM {MYSQL_DB}.tick_job WHERE date = %s", (date_str,))
